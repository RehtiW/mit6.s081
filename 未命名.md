...
1. Choosing the next thread to run:
 	2. To choose the next thread to run, we iterate through the ready list of threads. We choose the thread that has the
	highest effective priority.
3. Acquiring a Lock
 	4. When a thread tries to acquire a lock, we first test to see that the lock is free. If so, then we give the current
	...
	2. `int donate(struct lock *lock) {}`
 	...
 	5. If the thread that owns the lock is blocked and waiting for another lock (i.e. `waits_for` is not NULL), we
...
6. Releasing a Lock  
	...
	4. Reset the lock's `donated_priority` to 'PRI_MIN'.
	5. If the previous holder's updated priority is less than `threads_get_max_priority()`, then
	yield the previous holder's thread.
7. Computing the effective priority
 	8. We return the maximum of the thread's `priority` and it's `donated_priority`.
9. Priority scheduling for semaphores and locks, and condition variables
 	10. Whenever a semaphore, lock, or condition variable plans to release a thread, we will iterate over the threads in the waiting list to find the thread with the maximum effective priority.
	2. Unblock the thread with highest effective priority.
11. Changing thread's priority
 	12. Set the thread's priority to the input.
	2. If the current thread's priority is no longer the highest effective priority, then yield the current thread.
13. Finding the maximum priority (`int threads_get_max_priority(void) {}`)
...
1. Choosing the next thread to run
 ...
 2. If t<sub>0</sub> has a lower priority than the current running thread t<sub>c</sub>, we continue running t<sub>c</sub>.
 3. If t<sub>0</sub> has a higher or equal priority than the current running thread t<sub>c</sub>, t<sub>0</sub> is chosen to run (and t<sub>c</sub> is sent to the back of the queue (Round Robin Rule)). If the t<sub>c</sub> that is being replaced with t<sub>0</sub> is blocked or dying, we do not send it back to the ready queue.
...
4. Update priority queues
 ...
 5. Every tick we update the `recent_cpu` for the current thread.
 6. Every second we update the `recent_cpu` and the `load_average` according to the specification.
 7. Every 4 ticks we update the priorities (the above two operations occur first if they coincide).
 8. We start the update operation by recording the number of elements in each of the 64 priority queues. For each queue, we then only update the first few elements as dictated by the recorded number. This is done because the update operation may change the thread priorities and thus might have to be moved to another queue. Since the moved thread are pushed to the back of the queue, computing the priority of only the first few threads allows us to avoid recomputation of priorities for the moved threads.
 9. After we compute the priority for each thread, we move it to the back of another queue according to the new priority, only if the priority of the thread changed.
 10. We then update the priority for the blocked threads.
 11. We also update the priority for the current thread. (If the new priority is lower than the highes priority, it yields to the scheduler).
